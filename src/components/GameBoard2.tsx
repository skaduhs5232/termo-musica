...
function GameBoard({ 
  ...
  const handleCellPaste = (index: number, e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    e.preventDefault();
    const pastedText = e.clipboardData.getData('text').replace(/[^a-zA-Z0-9\s\-'&.,]/g, '').toUpperCase();
  ...
  const renderAttemptRow = (attempt: Attempt, attemptIndex: number) => {
      ...
      if (isCurrentRow) {
        ...
        cells.push(
          <input
            ...
            } text-center text-gray-900 dark:text-gray-100 focus:outline-none cursor-pointer`}
    ...
    for (let i = 0; i < cells.length; i += cellsPerRow) {
      ...
      rows.push(
        <div
          key={`${attemptIndex}-row-${Math.floor(i / cellsPerRow)}`}
          className={`flex justify-center ${isLongTitle ? 'gap-1' : 'gap-2'} mb-2`}
          role="row"
        >
          {rowCells}
        </div>
    ...
    return (
      <div
        key={attemptIndex}
        className="mb-4"
        aria-label={`Tentativa ${attemptIndex + 1}`}
      >
        {rows}
      </div>
    );
      ...
      <div className="mb-6 text-sm text-gray-600 dark:text-gray-400 text-center">
        ...
        {isGameActive && attempts.length < maxAttempts && (
          <div className="text-xs mt-1 space-y-1">
            <p className="text-blue-600 dark:text-blue-400">
              💡 Clique em qualquer caixa para digitar nela
            </p>
            <p className="text-green-600 dark:text-green-400">
              ⏎ Pressione Enter para enviar sua tentativa
            </p>
            <p className="text-purple-600 dark:text-purple-400">
              📋 Cole texto para preencher várias células
            </p>
          </div>
        )}
        ...
        {targetLength > 25 && (
          <p className="text-xs mt-2 text-orange-600 dark:text-orange-400">
            📏 Título longo - células adaptadas para melhor visualização
          </p>
        )}
      </div>
      
      <div className={`${targetLength > 35 ? 'max-w-full' : targetLength > 20 ? 'max-w-4xl' : 'max-w-2xl'} w-full px-4`}>
